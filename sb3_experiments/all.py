...
# Config: envs, algos and timesteps
env_configs = {
    "cartpole": {"id": "CartPole-v1", "algos": ["A2C", "PPO"], "timesteps": 50000},
    "pendulum": {"id": "Pendulum-v1", "algos": ["DDPG", "TD3", "SAC"], "timesteps": 50000},
}
...
class SaveLogsCallback(BaseCallback):
    ...
    def _on_training_end(self) -> None:
        df = pd.DataFrame({"reward": self.rewards, "length": self.lengths, "timestamp": self.times})
...
def aggregate_and_plot(results):
    ...
    for env in set(r["env"] for r in results):
        plt.figure(figsize=(10,6))
        plotted = False
        for r in results:
            if r["env"] != env: continue
            if not os.path.exists(r["log"]): continue
            df = pd.read_csv(r["log"])
            ...
            plotted = True
        if plotted:
            plt.title(f"Training curves (smoothed) on {env}")
            plt.xlabel("Episode")
            plt.ylabel("Episode reward (smoothed)")
            plt.legend()
            png = os.path.join(OUT_DIR, f"comparison_{env}.png")
            plt.savefig(png)
            plt.show()
    ...
    for key, cfg in env_configs.items():
        for algo in cfg["algos"]:
            try:
                ...
                results.append({"algo": algo, "env": cfg["id"], "log": csv_path, "model": model_path})
